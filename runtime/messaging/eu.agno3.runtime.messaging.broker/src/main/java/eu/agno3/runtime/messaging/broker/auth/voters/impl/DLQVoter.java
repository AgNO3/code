...
public class DLQVoter implements DestinationAccessVoter {
    ...
    public DestinationAccessVote vote ( ConnectionContext connContext, SecurityContext context, ActiveMQDestination dest, DestinationAccess access ) {
        ...
        if ( "ActiveMQ.DLQ".equals(dest.getPhysicalName()) && ( access == DestinationAccess.PRODUCE || access == DestinationAccess.CREATE ) ) { //$NON-NLS-1$
