...
import eu.agno3.runtime.jsf.types.uri.URLPostRestoreViewPhaseListener;
...
public class PrimeExceptionHandler extends ExceptionHandlerWrapper {
    ...
    private static final Logger LOG = Logger.getLogger(PrimeExceptionHandler.class);
        ...
        if ( exceptionQueuedEvents != null && exceptionQueuedEvents.iterator() != null ) {
            ...
            if ( unhandledExceptionQueuedEvents.hasNext() ) {
                try {
                    ...
                    if ( rootCause instanceof RuntimeIOException ) {
                        LOG.debug("IO failed", rootCause);
                    }
                    else {

                        ExceptionInfo info = createExceptionInfo(rootCause);

                        String reqUri = (String) context.getExternalContext().getRequestMap()
                        ...
                        if ( LOG.isDebugEnabled() ) {
                            LOG.debug("Request URI is " + reqUri);
                            LOG.debug("View ID is " + viewId);
                        }

                        try {
                            if ( viewId != null ) {
                                MDC.put("viewId", viewId);
                            }
                            if ( reqUri != null ) {
                                MDC.put("requestUri", reqUri);
                            }
                            LOG.warn("Caught exception " + rootCause.getMessage(), rootCause);
                        }
                        finally {
                            MDC.remove("viewId");
    ...
    protected void handleRedirect ( FacesContext context, Throwable rootCause, ExceptionInfo info ) throws IOException {
        ...
        context.getExternalContext()
                .redirect(context.getExternalContext().encodeActionURL(context.getExternalContext().getRequestContextPath() + errorPage));
        context.responseComplete();
    }
}
