...
import eu.agno3.fileshare.orch.common.config.desc.FileshareWebResourceLibraryDescriptor;
...
public class WebThemeSynchronizationHandler extends AbstractResourceLibraryConfigFilesSynchronizationHandler {

    private static final Logger log = Logger.getLogger(WebThemeSynchronizationHandler.class);
    ...
    public String getType () {
        return FileshareWebResourceLibraryDescriptor.RESOURCE_LIBRARY_TYPE;
    }
    ...
    protected ConfigFileManager getManager ( StructuralObjectReference service ) throws ResourceLibraryException {
        try {
            ...
            ConfigFilesManager cfm = this.configFilesManagerFactory.getForPath(
                PrefixUtil.resolvePrefix(this.executionConfig, sm.getConfigFilesPath()).toFile(),
                null,
                sm.getGroupPrincipal(),
                FileSecurityUtils.getGroupReadFilePermissions(),
                FileSecurityUtils.getGroupReadDirPermissions());
            return cfm.getCfgFileRoot("web"); //$NON-NLS-1$
        }
        catch (
            IOException |
    ...
    protected void updated ( StructuralObjectReference service, String hint ) {
        super.updated(service, hint);

        try {
            FileshareRuntimeServiceManager sm = this.serviceManager.getServiceManager(service, FileshareRuntimeServiceManager.class);
            sm.forceReloadAll("httpservice.webapp@fileshare"); //$NON-NLS-1$
        }
        catch ( ServiceManagementException e ) {
            log.warn("Failed to reload service", e); //$NON-NLS-1$
        }
