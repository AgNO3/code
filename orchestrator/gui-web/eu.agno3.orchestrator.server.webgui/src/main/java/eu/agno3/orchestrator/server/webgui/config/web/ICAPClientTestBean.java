 ...
 *
 */
@Named ( "web_icap_testBean" )
@ViewScoped
public class ICAPClientTestBean implements Serializable, ConfigTestReturnHandler {
    ...
    private static final Logger log = Logger.getLogger(ICAPClientTestBean.class);
    ...
    public void interact ( OuterWrapper<?> ctx, ConfigTestInteraction inter ) {
        ...
        if ( inter instanceof TrustCertificateInteraction ) {
            ...
            try {
                ...
                if ( ! ( cfg instanceof ICAPConfiguration ) ) {
                    return;
                }

                ICAPConfiguration lcf = (ICAPConfiguration) cfg;
