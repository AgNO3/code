 ...
 */
@ApplicationScoped
public class RealmMenuContributor extends AbstractMenuContributor {
    ...
    public List<WeightedMenuElement> getContributions ( StructuralObject selectedObject, StructuralObject refObject ) {
        ...
        if ( this.guiFeatures.getShowDevTools() ) {
            WeightedMenuItem itm = new WeightedMenuItem(301.0f, GuiMessages.get("menu.host.realms")); //$NON-NLS-1$
            itm.setOutcome("/realms/manage.xhtml" + makeObjectParameters(selectedObject, refObject)); //$NON-NLS-1$
