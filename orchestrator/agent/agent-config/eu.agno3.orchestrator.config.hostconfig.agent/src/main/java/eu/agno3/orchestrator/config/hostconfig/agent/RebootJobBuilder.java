...
}, property = "jobType=eu.agno3.orchestrator.config.hostconfig.jobs.RebootJob" )
@JobType ( RebootJob.class )
public class RebootJobBuilder extends AbstractSystemJobRunnableFactory<RebootJob> {
     ...
     */
    @Override
    protected void buildJob ( @NonNull JobBuilder b, @NonNull RebootJob j ) throws JobBuilderException {
        try {
            this.baseSystem.reboot(b, 5);
        }
        catch (
            UnitInitializationFailedException |
            BaseSystemException e ) {
            throw new JobBuilderException("Failed to initialize reboot job", e); //$NON-NLS-1$
        }
    }
