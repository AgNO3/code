 ...
 * 
 */
public class CleanUpPhaseExecutor implements PhaseExecutor {

    private static final Logger log = Logger.getLogger(CleanUpPhaseExecutor.class);

    private static final String CLEANUP_ERROR = "Cleanup error:"; //$NON-NLS-1$
    ...
    public Result execute ( ExecutionUnit<?, ?, ?> unit, Context context ) throws ExecutionException {
        try {
            unit.cleanup(context);
        }
        catch ( ExecutionException e ) {
            context.getOutput().error(String.format("%s %s", CLEANUP_ERROR, e.getMessage()), e); //$NON-NLS-1$
            log.trace(CLEANUP_ERROR, e);
