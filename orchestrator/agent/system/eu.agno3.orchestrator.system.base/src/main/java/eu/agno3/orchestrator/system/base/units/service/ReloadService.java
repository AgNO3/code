 ...
 * 
 */
public class ReloadService extends AbstractServiceExecutionUnit<ReloadService, ReloadServiceConfigurator> {

    /**
     * 
     */
    private static final long serialVersionUID = 2410843482317090606L;
    ...
    protected boolean doWithService ( Context context, Service s ) throws ExecutionException {
        try {
            ...
            throw new ExecutionException("Failed to reload service " + this.getServiceDescription(), e); //$NON-NLS-1$
        }
    }
    ...
    public ServiceResult execute ( Context context ) throws ExecutionException {
        context.getOutput().info("Reloading service " + this.getServiceDescription()); //$NON-NLS-1$
